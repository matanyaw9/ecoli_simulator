...
class VectorizedExperimentManager:
    def __init__(self, max_generations=config.MAX_NUMBER_OF_GENERATIONS,
        ...
        self.death_likelihood_vec = None    # easier way to approach fitness? (1 - fitness)

    def get_population_size(self):
        return self.death_likelihood_vec.size
    ...
    def generate_initial_population(self,
        ...
        self.death_likelihood_vec = np.full(shape=size_of_initial_population,fill_value=1-min(initial_beta_stored, 1),dtype=np.float64)
        self.beta_stored_vec = np.full(shape=size_of_initial_population,fill_value=initial_beta_stored,dtype=np.float64)
    ...
    def run_generation(self, print_info=False):
        ...
        survived = self.death_likelihood_vec < np.random.rand(self.death_likelihood_vec.size)
        # self.fitness_vec = self.fitness_vec[survived]
        self.death_likelihood_vec = self.death_likelihood_vec[survived]
        self.beta_stored_vec = self.beta_stored_vec[survived]

        # Followup data
        self.population_size_history.append(self.death_likelihood_vec.size)

        if (self.death_likelihood_vec.size == 0):
            print(
        ...
        self.total_beta_in_medium += self.beta_production * self.death_likelihood_vec.size
        ...
        multiplied = self.multiplication_rate > np.random.rand(
            self.death_likelihood_vec.size)
        self.beta_stored_vec[multiplied] /= float(2)

        # self.fitness_vec = np.minimum(
            # self.beta_stored_vec, 1)  # fitness is probability
        
        self.death_likelihood_vec = np.maximum(1 - self.beta_stored_vec, 0)  # death likelihood is 1 - fitness

        # self.fitness_vec = np.hstack(
            # (self.fitness_vec, self.fitness_vec[multiplied]))
        self.death_likelihood_vec = np.hstack(
            (self.death_likelihood_vec, self.death_likelihood_vec[multiplied]))
        self.beta_stored_vec = np.hstack(
            (self.beta_stored_vec, self.beta_stored_vec[multiplied]))

        if print_info:
            print(
                f"Generation: {self.current_generation_number}\t population size: {self.death_likelihood_vec.size}")
